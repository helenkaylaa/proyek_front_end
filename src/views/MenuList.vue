...
export default {
  ...
  data() {
    return {
      ...
      currentPage: 1,
      pageSize: 4,
      showSuccessPopup: false, // Menambahkan data untuk menampilkan/hide pop-up pesanan berhasil ditambahkan
    };
  },
  computed: {
    displayedMenus() {
  ...
  methods: {
    ...mapMutations(['tambahkanPesanan']),
    addToCart(menu) {
      this.tambahkanPesanan(menu);
      this.showSuccessPopup = true; // Menampilkan pop-up pesanan berhasil ditambahkan
      setTimeout(() => {
        this.hideSuccessPopup();
      }, 2000); // Mengatur waktu tampilan pop-up (dalam milidetik), dalam contoh ini 2 detik (2000 ms)
    },
    previousPage() {
    ...
    hideSuccessPopup() {
      this.showSuccessPopup = false; // Menyembunyikan pop-up pesanan berhasil ditambahkan
    },
  },
};
</script>
